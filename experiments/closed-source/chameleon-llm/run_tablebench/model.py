...
class solver:
    ...
    def build_prompt_for_pg(self):
        ...
        if choices:
            solution = "# Python Code, return 'ans' as a string selected from the options\n# Provide only the code block, no explanatory text"
        else:
            solution = "# Python Code, return 'ans' as a number\n# Provide only the code block, no explanatory text"
    ...
    def program_executor(self):
        if "program" in self.cache:
            ...
            print("No program in cache")
            return None, False
        match = re.search(r"```python\n(.*?)\n```", program, re.DOTALL)
        code = match.group(1) if match else program
        print(f"Extracted code:\n{code}")
        ans = safe_execute(code)
        print(f"Executor output: {ans}")
