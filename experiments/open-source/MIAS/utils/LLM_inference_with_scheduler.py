...
def LLM_inference_with_scheduler(prompt_PoT, prompt_text2sql, prompt_CoT, prompt_refine_PoT, prompt_refine_text2sql, llm, table, question, PoT_results, text2sql_results, CoT_results, scheduler_result , N = 3, sim_threshold = 0.9):
    ...
    if scheduler_result == 'PoT':
        ...
        if check_PoT_error(PoT_answer) != False and check_exact_match(PoT_answer.replace("[", "").replace("]", "").strip("'").strip('"'), CoT_results['N=0']["answer"].strip("'").strip('"')) == True:
            text2sql_results =  {
                    "N=0": {
                        "code": "<NOTHING>"
                    },
        ...
        if check_text2sql_error(text2sql_answer) != False and check_exact_match(text2sql_answer.replace("[", "").replace("]", "").strip("'").strip('"'), CoT_results['N=0']["answer"].strip("'").strip('"')) == True:
            PoT_results =  {
                    "N=0": {
                        "code": "<NOTHING>"
                    },
